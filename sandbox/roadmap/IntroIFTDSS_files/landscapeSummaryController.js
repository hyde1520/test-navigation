mainApp.controller("landscapeSummaryController", ["$scope", '$rootScope','$timeout','$location','$filter','$route','$window',"breadcrumbResource", "reportService", landscapeSummaryController])

function landscapeSummaryController($scope,$rootScope,$timeout,$location,$filter,$route,$window,breadcrumbResource, reportService){

	$rootScope.simple_nav_bar=true;
	$rootScope.no_breadcrumbs=true;

	$rootScope.navbar_title="Landscape Summary";
	$scope.reportType = $route.current.params.reportType;
	$rootScope.navbar_title=reportService.reportTypesAll[$scope.reportType];
	$scope.reportName = reportService.reportTypesAll[$scope.reportType];

	$scope.init = function()
	{
		$scope.layerNameFilter={"layerId": ""};
		$scope.landscapeId = $route.current.params.landscapeid;
		$scope.shapeId = $route.current.params.shapeId;
		$scope.lcpAttrIds = ['fm', 'cc', 'ch', 'cbh', 'cbd','asp', 'slp', 'elv'];
		$scope.allAttrIds = ['fm', 'cc', 'ch', 'cbh', 'cbd','asp', 'slp', 'elv', 'flm', 'spr', 'int', 'hta', 'crn'];
		$scope.basicOutputIds = ['flm', 'spr', 'int', 'hta', 'crn'];
		$scope.attrIds = $scope.allAttrIds;
		$scope.pdfPages = {
			// pending:,
			processed:[]
		};

		$scope.allLayerNameArr = {
			'fm': "Fuel Model (FBFM)",
            'cc': "Canopy Cover",
            'ch': "Stand Height",
            'cbh': "Canopy Base Height",
            'cbd': "Canopy Bulk Density",
            'asp': "Aspect",
            'slp': "Slope",
            'elv': "Elevation",
            'flm': 'Flame Length',
			'spr': 'Spread Rate',
			'int': 'Intensity',
			'hta': 'Heat/Area',
			'crn': 'Crown Fire'
		};
		$scope.landscapeOnlyNameArr = {
			'fm': "Fuel Model (FBFM)",
            'cc': "Canopy Cover",
            'ch': "Stand Height",
            'cbh': "Canopy Base Height",
            'cbd': "Canopy Bulk Density",
            'asp': "Aspect",
            'slp': "Slope",
            'elv': "Elevation"
        };
		$scope.basicOutputNameArr= {
			'flm': 'Flame Length',
			'spr': 'Spread Rate',
			'int': 'Intensity',
			'hta': 'Heat/Area',
			'crn': 'Crown Fire'
		};
		$scope.showBasicOutput = false;
		$scope.showAutoBasicOutput = false;

		if ($scope.reportType == "raut") {
			$scope.layerNameArr = $scope.allLayerNameArr;
			$scope.attrIds = $scope.allAttrIds;
		}
		else if ($scope.reportType == "rfbh") {
			$scope.layerNameArr = $scope.basicOutputNameArr;
			$scope.attrIds = $scope.basicOutputIds;
		}
		else {
			$scope.layerNameArr = $scope.landscapeOnlyNameArr;
			$scope.attrIds = $scope.lcpAttrIds;
		}
		$scope.queueMsg = "A request has been made for summary data. Check back later!";
		if (typeof($scope.summaryData) != 'undefined') {
			if ($scope.summaryData.responseMessage) {
				$scope.queueMsg = $scope.summaryData.responseMessage;
			}
			else if (typeof($scope.summaryData.landscapeName) != "undefined")
			{
				$scope.setTitle();

			 }
		}
		var pageName = $scope.reportName;
   		breadcrumbResource.setLeafView(pageName, "#");
  		$rootScope.navbar_title = pageName;

		// Create landscape filter list
		if (typeof($scope.landscape_layers) == "undefined" || $scope.landscape_layers == null) {
			$scope.landscape_layers = [];
		}
		$scope.landscape_layers.push( {"type":"allLcp", "name":"All Lcp"} );
		for (i=0; i < $scope.lcpAttrIds.length; i++) {
			var attribEnum = $scope.attrIds[i];
			$scope.landscape_layers.push( {"type":attribEnum, "name":$scope.layerNameArr[attribEnum]} );;
		}
		// Create model filter list
		if (typeof($scope.model_layers) == "undefined" || $scope.model_layers == null) {
			$scope.model_layers = [];
		}
		$scope.model_layers.push( {"type":"allMod", "name":"All Model"} );
		for (i=0; i < $scope.basicOutputIds.length; i++) {
			var attribEnum = $scope.basicOutputIds[i];
			$scope.model_layers.push( {"type":attribEnum, "name":$scope.basicOutputNameArr[attribEnum]} );;
		}
		console.log('INIT complete');
		if ($scope.reportType == 'rfbh'){
			$scope.layerNameFilter={"layerId": "allMod"};
		}else if ($scope.reportType == 'rlcp') {
			$scope.layerNameFilter={"layerId": "allLcp"};
		}
		// console.log($scope.reportType);

		//Set showAutoBasicOutput && showBasicOutput booleans for displaying Model Parameters
		
		for (var k in $scope.layerNameArr){
		    if ($scope.layerNameArr.hasOwnProperty(k) && !$scope.showBasicOutput) {
		    	$scope.showBasicOutput = $scope.isBasicOutput(k);
	         	// console.log("Key is " + k + ", value is " + $scope.layerNameArr[k]);
		    }
		    if ($scope.layerNameArr.hasOwnProperty(k) && !$scope.showAutoBasicOutput) {
		    	$scope.showAutoBasicOutput = $scope.isAutoBasicOutput(k);
	         	// console.log("Key is " + k + ", value is " + $scope.layerNameArr[k]);
		    }
		}

		// console.log($scope.showBasicOutput);
		// console.log($scope.showAutoBasicOutput);

	} //end init()

	$scope.getLayerName = function(layerId) {
		return $scope.layerNameArr[layerId];
	}
	$scope.showLandscapeChars = function() {
		if (($scope.reportType == 'rlcp') ||
			($scope.reportType == 'raut')){
			return true;
		}
		return false;
	}
	$scope.showModelChars = function() {
		if (($scope.reportType == 'rfbh') ||
			($scope.reportType == 'raut')){
			return true;
		}
		return false;
	}

	$scope.setTitle = function() {
		$scope.landscapeName = "Auto97th - ";
		if ($scope.reportType) {
			if ($scope.reportType == 'rlcp') {
				$scope.landscapeName = "Landscape Summary - ";
			}
			else if ($scope.reportType == 'rfbh') {
				$scope.landscapeName = "Landscape Fire Behavior - ";
			}
		}
		$scope.landscapeName += $scope.summaryData.landscapeName  ;

		if ($scope.summaryData.shapeName != null) {
			$scope.shapeName = $scope.summaryData.shapeName;
			$scope.landscapeName += ( " - " + $scope.shapeName);
		}

		breadcrumbResource.setLeafView( $scope.landscapeName + "-" +  $scope.reportName , '#');
		$scope.landscapeSource = "    (" + $scope.summaryData.landscapeSource + ") " + $scope.summaryData.acreage.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " acres" ;
	}

	$scope.getImageUrl = function() {
        layerEnum = this.tableId;
        var layerSummaryData = $scope.summaryData.layerSummary;
        var imageUrl = layerSummaryData[layerEnum].summaryImage;
        if (typeof(imageUrl) != 'undefined') {
        	imageUrl = imageUrl.replace(/\\/g, "/");
        }
        return imageUrl;
    }

	$scope.openBigImage = function() {
	/*	imgsrc = "images/testLandscape.gif";
		layerEnum = this.tableId;
		var layerSummaryData = $scope.summaryData.layerSummary;
		//layerSummaryData[layerEnum].summaryImage;
		viewwin = window.open(imgsrc,'viewwin', 'width=600,height=300'); */
	}
	
	$scope.downloadImg = function() {
		layerEnum = this.tableId;
        var layerSummaryData = $scope.summaryData.layerSummary;
        var imageUrl = layerSummaryData[layerEnum].summaryImage;
        if (typeof(imageUrl) != 'undefined') {
        	imageUrl = imageUrl.replace(/\\/g, "/");
        }
        // console.log(imageUrl);
        download(imageUrl);
	}


	// The resolve (in app.js) for this page will use the landscapeResource to
	// make the REST call for summary data.
	if ($route.current.locals.summaryData) {
			$scope.summaryData = $route.current.locals.summaryData.data;
		 if (($scope.summaryData == null) || (typeof($scope.summaryData.layerSummary) == "undefined" )){
			 console.log("Landscape summary queued");
			 $scope.summaryQueued = true;
			 $scope.summaryExists = false;
		 }
		 else {
			 $scope.summaryExists = true;
			 $scope.summaryQueued = false;
		 }
	}
	$scope.fetchSummaryDataForRefresh = function()
	{
		$scope.landscapeId = $route.current.params.landscapeid;
		$scope.shapeId = $route.current.params.shapeId;

		reportService.getLandscapeSummary($scope.landscapeId, $scope.shapeId, "0").then(function(response){
			$scope.summaryData = response;
		},function(errorResponse){
			// console.log
		});
	}

	$scope.refreshPage = function () {
		//$scope.fetchSummaryDataForRefresh();
		$route.reload();
	};

	$scope.setFilter = function(layerEnum) {
		$scope.layerNameFilter = {"layerId": layerEnum.type};
	}

	$scope.isALayerFilterMatch = function(layerEnum) {
		if ($scope.layerNameFilter.layerId == 'allLcp')  {
			if (typeof $scope.landscapeOnlyNameArr[layerEnum] != 'undefined') {
				return true; // All LCP types
			}
		}
		else if ($scope.layerNameFilter.layerId == 'allMod')  {
			if (typeof $scope.basicOutputNameArr[layerEnum] != 'undefined') {
				return true; // All model types
			}
		}
		if ($scope.layerNameFilter.layerId == '')  return true;
		if ($scope.layerNameFilter.layerId == layerEnum)  return true;
		return false;
	}

	$scope.isBasicOutput = function(basicOutputId) {
    	return ($scope.basicOutputIds.indexOf(basicOutputId) !== -1);
	}

	$scope.isAutoBasicOutput = function(basicOutputId) {
		return ($scope.reportType == "raut" && $scope.isBasicOutput(basicOutputId)); 
	}

	$scope.landscapeName = "";
	$scope.landscapeSource = "";
	$scope.aoiTag = '';

	// Don't load chart data and draw charts until table with ng-repeat is in the DOM
	angular.element(document).ready(function ()
	{
		console.log($scope.summaryData);

		$scope.aoiTag = (typeof($scope.summaryData.shapeName) == 'undefined')?' In LCP':' In AOI';

		if ((typeof($scope.summaryData) != "undefined" ) &&
				(typeof($scope.summaryData.landscapeName) != "undefined"))
		{
			$scope.setTitle();
		 }
		else if (typeof($scope.summaryData.layerSummary) == "undefined" ){
			 console.log("Landscape summary queued");
			 $scope.summaryQueued = true;
			 $scope.summaryExists = false;
			 $scope.queueMsg = $scope.summaryData.responseMessage;
		 }
		$scope.drawHighCharts();

	});

	function setDisplayStyle(elementId, styleVal) {
		 var element = document.getElementById(elementId);
		 if (element && element.style) {
			 element.style.display = styleVal;
		 }
	 }

	 // This function will be called after page is loaded to render the
	 // pie chart, data table and column chart (and eventually image)
	 $scope.drawHighCharts = function() {
		 console.log('Drawing High Charts');
 	 	if (typeof($scope.summaryData.layerSummary) == "undefined" ){
 		 	console.log("Landscape summary queued");
 		 	$scope.summaryQueued = true;
 		 	$scope.summaryExists = false;
 		 	$scope.queueMsg = $scope.summaryData.responseMessage;
 		}else{
 			var layerSummaryData = $scope.summaryData.layerSummary;
 			if ($scope.reportType == "raut") {
 				$scope.layerNameArr = $scope.allLayerNameArr;
 				$scope.attrIds = $scope.allAttrIds;
 			}else if ($scope.reportType == "rfbh") {
 				$scope.layerNameArr = $scope.basicOutputNameArr;
 				$scope.attrIds = $scope.basicOutputIds;
 			}else {
 				$scope.layerNameArr = $scope.landscapeOnlyNameArr;
 				$scope.attrIds = $scope.lcpAttrIds;
 			}
 		}
 		// For each layer that we have summary data for, show it on the page.
 		//$scope.layerNameArr
 		// console.log($scope.layerNameArr);
 		for ( var layer in $scope.layerNameArr) {
 			if (typeof(layerSummaryData) == 'undefined') {
 				console.log("Layer summary data does not exist.  Check for errors from the server " );
 			 	break;
 			}
 		 	if (typeof(layerSummaryData[layer]) == "undefined") {
 			 	console.log("Layer " + layer + "not supported or has no summary data");
 			 	continue;
 		 	}
 			// var layerName = layerSummaryData[layer].layerName;
 			var unitStr =  (typeof(layerSummaryData[layer].unit) != 'undefined') ?
 				" (" + layerSummaryData[layer].unit.trim() + ")" :
 				"";
 			var chartObj = {
 				layerName : layerSummaryData[layer].layerName,
 				unitStr : unitStr,
 				layer: layer,
 				layerType: replaceAll(layerSummaryData[layer].layerName, "_", " ") +unitStr
 			}
 			// console.log("Layer " + layer);
 			// console.log(layerSummaryData[layer]);

		 	/*
				Onscreen chart title should include:
				Source Landscape Name:
				Source Landscape Acres:
				Area of Interest Name:
				Area of Interest ACres:
				Title Example: Canopy Cover Data Summary for Area of Interst {AOIname} within {Landscape Name}

				if $scope.showBasicOutput == true
				
				For Model Runs
				Source Landscape Name:
				Source Landscape Acres:
				Area of Interest Name:
				Area of Interest Acres:
				Model name: 
				Model Output Name:
				Title Example: {Flame Length} Data Summary for Area of Interest {AOI name} within {Landscape Name}
		  	*/

 			var aoiInterest = (typeof($scope.summaryData.shapeName) == 'undefined') ? '' : 'for Area of Interest "' + $scope.summaryData.shapeName + '" ';
 			var title = chartObj.layerType + ' Data Summary ' + aoiInterest +  'within "' + $scope.summaryData.landscapeName+ '" Landscape';
 			var titleData = 
 				'Source Landscape Name: '+ $scope.summaryData.landscapeName + '<br>' +
				'Landfire Version: '+  $scope.summaryData.landscapeSource + '<br>' +
				'Source Landscape Acres: '+ $scope.summaryData.acreage.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","); 
 			if(typeof($scope.summaryData.shapeName) != 'undefined'){
 				titleData += '<br>' + 'Area of Interest Name: ' + $scope.summaryData.shapeName;
 				if(typeof($scope.summaryData.aoiAcreage) != 'undefined'){
					titleData += '<br>' + 'Area of Interest Acres: ' + $scope.summaryData.aoiAcreage.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				}
 			}
 			if($scope.showBasicOutput){
 				titleData += '<br>' + 'Model Name: ' + $scope.summaryData.basicRunBO.runName;
 				// titleData += '</br>' + 'Model Output Name: ' + $scope.summaryData.shapeName;
 			}
 			// Build bar chart
 			var columnChartData = layerSummaryData[layer].barChartEntries;
 			$scope.drawSummaryBarChart(title,titleData,chartObj, columnChartData,layerSummaryData[layer]);

 			// Build Pie chart
 			var pieChartData = layerSummaryData[layer].pieChartEntries;
 			// var pieChartColors = layerSummaryData[layer].pieChartColors;
 			$scope.drawSummaryPieChart(title,titleData,chartObj, pieChartData,layerSummaryData[layer]);

 			// Build data table
 		    var dataTable = layerSummaryData[layer].dataTableEntries
 		    $scope.drawSummaryDataTable(title,titleData,chartObj, dataTable,layerSummaryData[layer]);
			//
			// 		console.log($scope.layerNameFilter.layerId);
			// console.log(layer);
			// 	// var layerSummaryData = $scope.summaryData.layerSummary;
			// 	//     var dataTable = layerSummaryData[layer].dataTableEntries
 		}
 	 	$scope.summaryExists = true;
 	 	$scope.summaryQueued = false;
     	$('#loader').hide();
	 }

	$scope.drawSummaryPieChart = function(title,titleData,chartObj, chartData,layerSummaryData) {

		 var dataLength = chartData.length;
		 var layerStr = replaceAll(chartObj.layerName, "_", " ");
		 var landscapeInfo = $scope.summaryData.landscapeName + " (" +  $scope.summaryData.landscapeSource + ")";
		 var xDataSeries = [];
		 var xDataColors = [];
		 var chart_div = 'piechart_' + chartObj.layer;
		 var landscapeInfo = $scope.summaryData.landscapeName + " (" +  $scope.summaryData.landscapeSource + ")";
		 for (var i = 0; i < dataLength; i++) {
		 	xDataSeries.push({
		 		name: chartData[i].xShortLabel,
		 		y:chartData[i].pixelCount
		 	});
		 	xDataColors.push(chartData[i].color);
		 }
		 // console.log(title);
		 // console.log(chartData);
		 // console.log(xDataSeries);
		 // console.log(xDataColors);
		 var options = {
		 	chart: {
		 		type:'pie'
		 	},
		 	title: {
		 		align: "left",
		 		text: title,
		 		style: {
		 			fontSize: '21px',
		 			fontWeight:'900',
		 		},
		 	},
		 	subtitle:{
		 		align: "left",
		 		text:  titleData,
		 		style: {
		 			fontSize: '11px',
		 			fontWeight:'900',
		 			fontStyle:'italics',
		 		},
		 	},
		 	tooltip: {
		 		headerFormat: '<span style="font-size:19px;color:black">{point.key}</span><br/>',
		 		// pointFormat: '<span style="color:black">Percentage</span>: <b>{point.y}%</b><br/>'
		 		pointFormat: 'Pixels:<b>{point.y}</b><br/>{series.name}: <b>{point.percentage:.0f}%</b>'
		 	},
		 	colors: xDataColors,
		 	plotOptions: {
				pie: {
				   	borderWidth: .3,
				   	borderColor: 'black',
				   	size: '100%',
				  	data: xDataSeries,
			   	}
		 	},
		 	
		 	series: [
				{
	                type: 'pie',
					name:'Percentage',
		 			showInLegend: true,
					dataLabels: {
						useHtml:true,
		 				formatter: function() {
		 					// return  this.percentage < 7 ? '' :  this.point.name + '<br/>' + Highcharts.numberFormat(this.percentage,1) + ' %';
		 					return  this.percentage < 7 ? '' :  Highcharts.numberFormat(this.percentage,0) + ' %';
		 				},
	 					distance: -30,
		 				// color:'black',
					},
	            },
				/*
				// Outside Labels	
				{
	                type: 'pie',
					name:'Percentage',
		 			showInLegend: false,
					dataLabels: {
						useHtml:true,
                    	connectorColor: "black",
		 				formatter: function() {
		 					// return  this.percentage >= 7 ? null :  this.point.name + '<br/>' + Highcharts.numberFormat(this.percentage,1) + ' %';
		 					return  this.percentage >= 7 ? null :  Highcharts.numberFormat(this.percentage,1) + ' %';
		 				},
	 					distance: 20,
                        color:'black',
					},
	            },*/
		 	],
		 }
		 if ($('#'+chart_div).length) {
		 	Highcharts.chart(chart_div, options );
		 }else {
		 	console.log("Div element not found for '" + columnchart_div_name + "'");
		 }
	  }

	$scope.drawSummaryBarChart = function(title,titleData,chartObj,chartData,layerSummaryData){
  	 	// console.log(chartData);
		  var dataLength = chartData.length;
		  var xDataSeries = [];
		  var xDataCatagories = [];
		  var xDataColors = [];
		  var chart_div = 'columnchart_' + chartObj.layer;
		  var landscapeInfo = $scope.summaryData.landscapeName + " (" +  $scope.summaryData.landscapeSource + ")";
  	 	  var subtitleExt = (chartObj.layer == 'fm')? 'Distribution under 1% not shown':'';
		  for (var i = 0; i < dataLength; i++) {
		 	 xDataCatagories.push(chartData[i].xShortLabel);
		 	 xDataSeries.push(chartData[i].percentage);
		 	 xDataColors.push(chartData[i].color);
		  }
		  // console.log(xDataCatagories);
		  // console.log(xDataSeries);
		  // console.log(xDataColors);
		  var options = {
		 	 chart: {
		 		 type: 'column'
		 	 },
		 	 legend: {
		 		 enabled: false
		 	 },
		 	 title: {
		 		align: "left",
		 		text: title + '<br>',
	 		 	style: {
		 			fontSize: '21px',
		 			fontWeight:'900',
	 		 	},
		 	 },
		 	 subtitle:{
		 		 align: "left",
		 		 text:  titleData + '<br>' + subtitleExt ,
		 		 style: {
	 			 	fontSize: '11px',
	 			 	fontWeight:'900',
	 			 	fontStyle:'italics',
		 		 },
		 	 },
		 	 tooltip: {
		 		 headerFormat: '<span style="font-size:19px;color:black">{point.x}</span><br>',
		 		 pointFormat: '<span style="color:black">Percentage</span>: <b>{point.y}%</b><br/>'
		 	 },
		 	 xAxis: {
		 		 categories: xDataCatagories,
		 		 title: {
	 			 	text: layerSummaryData.yAxisLabel,
		 		 },
		 	 },
		 	 yAxis: {
		 		 title: {
		 			text: 'Percent'
		 		 }
		 	 },
		 	 colors: xDataColors,
		 	 plotOptions: {
		 		 column: {
		 			 colorByPoint: true
		 		 }
		 	 },
		 	 series: [
		 		 {
		 			 data: xDataSeries
		 		 }
		 	 ],
		  }
		  if ($('#'+chart_div).length) {
		 	 Highcharts.chart(chart_div, options );
		  }else {
		 	 console.log("Div element not found for '" + chart_div + "'");
		  }
	}

	$scope.drawSummaryDataTable = function(title,titleData,chartObj, chartData,layerSummaryData) {
		// console.log(title);
		var landscapeInfo = $scope.summaryData.landscapeName + " (" +  $scope.summaryData.landscapeSource + ")";
		$('#tablechartTitle_'+chartObj.layer).html(title);
		$('#tablechartSubTitle_'+chartObj.layer).html(titleData);
		var dataSet = [];
		var dataLength = chartData.length;
		var datatable_div = 'tablechart_' + chartObj.layer;
		var acres = 0;
		for (var i = 0; i < dataLength; i++) {
			dataSet.push([
				chartData[i].xLabel,
				chartData[i].pixelCount,
				chartData[i].acres,
				chartData[i].percent,
			]);
			acres = acres + chartData[i].acres;
		}

		/*if($scope.reportType == "raut"){
			$('#tablechartSubAcres_'+chartObj.layer).html('<br>'+'Total Acres  '+ acres.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
		}*/
		// console.log( $('#'+datatable_div));
		if ( $('#'+datatable_div).length) {
			// console.log(jQuery.fn.DataTable);
			// console.log(jQuery().jquery);
			$('#'+datatable_div).DataTable({
			   	data: dataSet,
        		deferRender: true,
        		order: [],
        		"language": {
		            "decimal": ",",
		            "thousands": "."
		        },
			   	columns: [
				   	{ title: chartObj.layerType, orderable: false},
				   	{ title: "Pixel Count (freq)",render: $.fn.dataTable.render.number(',', '.')  },
				   	{ title: "Acres"+$scope.aoiTag,render: $.fn.dataTable.render.number(',', '.') },
				   	{ title: "Percent"+$scope.aoiTag },
			   	],
                dom: '<"row"B <"col-xs-12"l> <"col-xs-12"r> <"col-xs-12"t><"col-xs-12"i> <"col-xs-12"p> >',
                drawCallback: function(settings) {
				    var pagination = $(this).closest('.dataTables_wrapper').find('.dataTables_paginate');
				    pagination.toggle(this.api().page.info().pages > 1);
			  	},
        		scrollX: true,
        		autoWidth: true,
				scrollCollapse: true,
				/*
				// Enable Scroll
				scrollY: "400px",
				scrollCollapse: true,
				paging: false,
				dom: 'Blrtip',
				*/
		        buttons: [
		            'copy',
					{
					   	extend: 'csv',
						filename:title,
				   	},
					{
					   	extend: 'excel',
						filename:title,
				   	},
					{
					   	extend: 'pdf',
						filename:title,
						title:title,
				   	},
					{
					   	extend: 'print',
						title:title,
				   	},
		        ]
			});
	    } else {
	    	console.log("Div element not found for 'tablechart" + layer + "'");
	    }
  	}


	$scope.savePieAsImg = function(){
		$('#'+'piechart_' + this.tableId).highcharts().exportChart();
	}

	$scope.saveBarAsImg = function(){
		$('#'+'columnchart_' + this.tableId).highcharts().exportChart();
	}

	function getImageData(svg) {
		var canvas = document.createElement('canvas');
		canvas.setAttribute('width', 800);
		canvas.setAttribute('height',400);
		canvg(canvas, svg);
		var imgData = canvas.toDataURL("image/png");
		return imgData;
	}

	function convertImgToDataURLviaCanvas(url, rowIndex, outputFormat) {
		  var img = new Image();
		  img.crossOrigin = 'Anonymous';
		  img.onload = function() {
		    var canvas = document.createElement('CANVAS');
		    var ctx = canvas.getContext('2d');
		    var dataURL;
		    canvas.height = this.height;
		    canvas.width = this.width;
		    ctx.drawImage(this, 0, 0);
		    dataURL = canvas.toDataURL(outputFormat);
			$scope.pdfPages.pending[rowIndex].data = dataURL;
		    canvas = null;
		  };
		  img.src = url;
	}

	function convertFileToDataURLviaFileReader(url,rowIndex) {
	  	var xhr = new XMLHttpRequest();
	  	xhr.onload = function() {
	    	var reader = new FileReader();
		    reader.onloadend = function() {
				_processPdfPage(reader.result,rowIndex);
		    }
	    	reader.readAsDataURL(xhr.response);
	  	};
	  	xhr.open('GET', url);
	  	xhr.responseType = 'blob';
	  	xhr.send();
	}

	function convertFileToDataURLviaFileReader(url,rowIndex) {
	  	var xhr = new XMLHttpRequest();
	  	xhr.onload = function() {
	    	var reader = new FileReader();
		    reader.onloadend = function() {
				_processPdfPage(reader.result,rowIndex);
		    }
	    	reader.readAsDataURL(xhr.response);
	  	};
	  	xhr.open('GET', url);
	  	xhr.responseType = 'blob';
	  	xhr.send();
	}

	function _addPdfPage(data){
		// Push new element into pdfPages Pending
		if(!$scope.pdfPages.hasOwnProperty("processed")){
			$scope.pdfPages.processed = [data];
			$rowIndex = 0;
		}else{
			var rowIndex = $scope.pdfPages.processed.push(data)-1;
		}
		return rowIndex;
	}

	function _processPdfPage(data,rowIndex){
		// console.log($scope.pdfPages.pending);
		// console.log('incoming index=> '+ rowIndex);
		//set Data Element
		$scope.pdfPages.processed[rowIndex].data = data;
		//Now remove element from pending array
		var index = $scope.pdfPages.pending.indexOf(rowIndex);
		// console.log('suggested removing index=> '+index);
		if (index > -1) {
			// console.log('removing index=> '+index);
		    $scope.pdfPages.pending.splice(index, 1);
		}
		// console.log($scope.pdfPages.pending);
		_printPage();
	}

	function _printPage(){
		//See if this was the last element to be processed.
		if($scope.pdfPages.pending.length > 0){
			return false;
		}
		//https://github.com/MrRio/jsPDF/blob/176a605da06c7f18d9a00f3c60c897f1fa61c5e1/jspdf.js
		var doc = new jsPDF('p', 'pt', 'letter');

		doc.page = 1; // use this as a counter.


		//Set Line Counter
		var line = 150;
		//Start With Logo

		//jsPDFAPI.addImage = function(imageData, format, x, y, w, h, alias, compression, rotation) {
		var iftdssLogo = 'data:image/png;base64,';
		doc.addImage(iftdssLogo, 'PNG', 165, line);

		//Move Line down 100pts
		line = line+100;

		//Set Bolder Font
		doc.setFontSize(20).setTextColor(40).setFontStyle('bold');

		//Add Title Function is located in highcharts-iftdss.js

		/*
			Report:  Auto 97th
			Landfire Version:
			Landscape: Upland Island
			Landscape Acres:
			Area of Interest: Upland Island Burn
			Area of Interest Acres:
		
	}
		*/
		var reportType = 'Auto97th';
		if ($scope.reportType) {
			if ($scope.reportType == 'rlcp') {
				reportType = "Landscape Summary";
			}else if ($scope.reportType == 'rfbh') {
				reportType = "Landscape Fire Behavior";
			}
		}


		var landscapeName = replaceAll('Landscape Name: '+ $scope.summaryData.landscapeName, "-", " ");
		/*var titleText = [];
		titleText.push('Report: '+ reportType);
		titleText.push('Landfire Version: ' +  $scope.summaryData.landscapeSource);
		titleText.push(replaceAll('Landscape: '+ $scope.summaryData.landscapeName, "-", " "));
		titleText.push('Landscape Acres: '+ $scope.summaryData.acreage);
		if(typeof($scope.summaryData.shapeName) != 'undefined'){
			titleText.push('Area of Interest: '+ $scope.summaryData.shapeName);
		}*/


		doc.myText('Report: '+ reportType,{align: "center"},0,line);
		line = line+25;
		doc.myText('Landfire Version: ' +  $scope.summaryData.landscapeSource,{align: "center"},0,line);
		line = line+25;

		var title = ReportHelpers.strToNewLine(landscapeName);
		var tL = title.length;
		for (var i = 0; i < tL; i++) {
			doc.myText(title[i],{align: "center"},0,line);
			line = line+25;
		};
		doc.myText('Landscape Acres: '+ $scope.summaryData.acreage.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),{align: "center"},0,line);
		line = line+25;

		if(typeof($scope.summaryData.shapeName) != 'undefined'){
			doc.myText('Area of Interest: '+ $scope.summaryData.shapeName,{align: "center"},0,line);
			line = line+25;
		}

		/*
		var title = ReportHelpers.strToNewLine(replaceAll($scope.landscapeName, "-", " ")+" Report");
		console.log(title,$scope.landscapeName);
		var tL = title.length;
		for (var i = 0; i < tL; i++) {
			doc.myText(title[i],{align: "center"},0,line);
			line = line+25;
		};
		*/

		// doc.myText($scope.landscapeName +" Report",{align: "center"},0,line);
		
		//Add Title if title is set 
		if(typeof($scope.allLayerNameArr[$scope.layerNameFilter.layerId]) == 'undefined'){
			var title = $scope.summaryData.landscapeName+' Report';
			line = line+20;
		}else{
			// line = line+25;
			var title = $scope.summaryData.landscapeName + ' ' +$scope.allLayerNameArr[$scope.layerNameFilter.layerId];
			doc.myText('Section: '+$scope.allLayerNameArr[$scope.layerNameFilter.layerId],{align: "center"},0,line);
			line = line+20;
		}

		//Add prepared "For" signature
		doc.setFontSize(10).setFontStyle('italic');
		doc.myText('Prepared for: '+$scope.loggedInName,{align: "center"},0,line);
		line = line+15;
		doc.myText(new Date(Date.now()).toLocaleString(),{align: "center"},0,line);
		doc.setFontStyle('normal');
		
		//Set footer Pages Counter;
		var pl = $scope.pdfPages.processed.length;
		var totalPages = pl+1;
		for (var i = 0; i < pl; i++) {
			var el = $scope.pdfPages.processed[i];
			doc.page ++;
			doc.addPage();
			var textStart = 40;

			// SET HEADER
			if(typeof(el.sectionInfo.tableName) != 'undefined'){
				doc.setFontSize(20).setTextColor(40).setFontStyle('normal');
				doc.text(el.sectionInfo.tableName, 50, textStart);
				textStart = textStart+15;
			}

			/*doc.setFontSize(8);
			doc.text('Report: '+ reportType, 50, textStart);
				textStart = textStart+9;
			doc.text('Landfire Version: ' +  $scope.summaryData.landscapeSource, 50, textStart);
				textStart = textStart+9;
			doc.text(landscapeName, 50, textStart);
				textStart = textStart+9;
			doc.text('Landscape Acres: '+ $scope.summaryData.acreage.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), 50, textStart);
			if(typeof($scope.summaryData.shapeName) != 'undefined'){
				textStart = textStart+9;
				doc.text('Area of Interest: '+ $scope.summaryData.shapeName, 50, textStart);
			}
			textStart = textStart+15;*/

			
			/*if(typeof(el.sectionInfo.landscapeName) != 'undefined'){
				doc.setFontSize(7);
				doc.text(el.sectionInfo.landscapeName, 50, 55);
			}*/


			// doc.setFontStyle('italic');
			// doc.text(el.sectionInfo.reportName, 50, 65);
			// doc.text('Distribution under 1% not shown', 50, 65);
			doc.setFontSize(20).setTextColor(40).setFontStyle('normal');

			// SET FOOTER
			var str = "Page " + doc.page  + " of " +  totalPages;
			doc.setFontSize(10);
			var bottom = doc.internal.pageSize.height - 10;
			var right = doc.internal.pageSize.width - 10;

			doc.text(str, 50, bottom);
			doc.addImage(iftdssLogo, 'PNG', right - 85, bottom -13, 60, 15);

			// ADD REPORT ELEMENT TO PAGE
			if(el.type == 'h'){
				//Process Html
        		doc.fromHTML(el.data, 50, textStart);
				// console.log(el.type);
			}else if (el.type == 'i') {
				//JPEG for IFTDSS
				var margin = 50;
				var width = doc.internal.pageSize.width - (margin*2);
				doc.addImage(el.data, 'PNG', margin, textStart,width,width);
				// doc.addImage(el.data, 'PNG', 50, 80, 500, 350);
				// console.log(el.type);
			}else if (el.type == 'c') {
				doc.addImage(el.data, 'PNG', 50, textStart, 500, 417);
				// console.log(el.type);
			}else if (el.type == 'd') {
        		var resp = doc.fromHTML(el.data.html, 50, textStart);
        		doc.autoTable(el.data.table.columns, el.data.table.rows,{
					theme: 'grid',
    				startY:resp.y,
					headerStyles: {
				        fillColor: [46, 62, 78],
				        textColor: [255, 255, 255],
				    },
			        styles: {
			        	overflow: 'linebreak', 
    					// columnWidth:50,
			        },
			        columnStyles: {
			         	0: {columnWidth: 100},
			     	}
				});
				// console.log(el.type);
			}else if (el.type == 't') {
				// Get Data
				// Table Data
				//https://github.com/simonbengtsson/jsPDF-AutoTable/blob/409ae6a0f6b21af849ff950e8404dc795c3153e1/README.md#options
				doc.autoTable(el.data.columns, el.data.rows,{
					theme: 'grid',
    				margin: {top: textStart},
					headerStyles: {
				        fillColor: [46, 62, 78],
				        textColor: [255, 255, 255],
				    },
			        styles: {
			        	overflow: 'linebreak', 
			        },
				});
				// console.log(el.type);
				// console.log(el.data);
			}
		}
		doc.save(title+'.pdf');
		$scope.pdfPages = {pending:[],processed:[]};
     	$('#loader').hide();
	}

	$scope.printPage =  function(){
		$('#loader').show({
			complete:function(){
				//Find all visible .summary-report-table
				$('.basic-meta-container:not(.ng-hide), .summary-report-table:not(.ng-hide)').each(function(i, o) {
					//Get Header Info for this Section
					var $headerContainer = $(this).siblings( ".header-group" );
					sectionInfo = {
						tableName : $headerContainer.find("[data-el='tableName']").data('val'),
						landscapeName : $headerContainer.find("[data-el='landscapeName']").text(),
						reportName : $headerContainer.find("[data-el='reportName']").data('val'),
					};
					// console.log(sectionInfo);
					//Set Process Pending List
					$(this).find('.stage-show:not(.ng-hide)').each(function(ii, oo) {
						//DataTables creates two Elements so make sure the element truly exists
						var idd = $(this).attr('id');
						// console.log(idd);
						if(typeof(idd) != 'undefined'){

							//Get Element type
							/*
							* h = Html
							* i = Image
							* c = Chart (High Charts)
							* t = Data Tables
							*/
							var type = (idd == 'basicTableOutput') ? 'd' : $(this).attr('data-type');
							// var type = $(this).attr('data-type');
							// console.log(idd+'=>'+type);
							var rowIndex = _addPdfPage({
								id: idd,
								type: type,
								data:null,
								sectionInfo:sectionInfo,
							});
							// console.log(rowIndex);
							if(!$scope.pdfPages.hasOwnProperty("pending")){
								$scope.pdfPages.pending = [rowIndex];
							}else{
								$scope.pdfPages.pending.push(rowIndex);
							}
						}
					});
				});

				//Process Pending List
				// console.log($scope.pdfPages.pending);
				// console.log($scope.pdfPages.processed);
				var processedLength = $scope.pdfPages.processed.length;
				// console.log(processedLength);
				for (var rowIndex = 0; rowIndex < processedLength; rowIndex++) {
					var obj = $scope.pdfPages.processed[rowIndex];
					// console.log(obj);
					// console.log(rowIndex);
					if(obj.type == 'h'){
						//Process Html
						var $clone = $('#'+obj.id).clone();
						if(obj.id == 'basicTableOutput' && !$scope.showAutoBasicOutput){
							$clone.find("#autoBasicOutput").remove();
    					}
						$clone = $clone.wrap('<div/>').parent().html();
						// console.log($clone);
						_processPdfPage($clone,rowIndex);
						// console.log(type);
					}else if (obj.type == 'd') {
						var $clone = $('#'+obj.id).clone();
						if(obj.id == 'basicTableOutput'){
							$clone.find("#fmOutput").remove();
    					}
    					if(obj.id == 'basicTableOutput' && !$scope.showAutoBasicOutput){
							$clone.find("#autoBasicOutput").remove();
    					}

    					var $el = {
							html:  $clone.wrap('<div/>').parent().html(),
							table: undefined
						}

						var rows = [];
						var columns = [
					   		'Fuel \nModel',
					   		"1 Hr \nFuel Moisture", 
					   		"10 Hr \nFuel Moisture", 
					   		"100 Hr \nFuel Moisture", 
					   		"Live Herbaceous \nFuel Moisture",
					   		"Live Woody \nFuel Moisture",
					   	];
					   	var dataLength = $scope.summaryData.basicRunBO.fuelMoistures.length;
						for (var i = 0; i < dataLength; i++) {
							rows.push([
								$scope.summaryData.basicRunBO.fuelMoistures[i].modelCodeOrName,
								$scope.summaryData.basicRunBO.fuelMoistures[i].fm1Hr,
								$scope.summaryData.basicRunBO.fuelMoistures[i].fm10Hr,
								$scope.summaryData.basicRunBO.fuelMoistures[i].fm100Hr,
								$scope.summaryData.basicRunBO.fuelMoistures[i].fmLiveHerbaceous,
								$scope.summaryData.basicRunBO.fuelMoistures[i].fmLiveWoody,
							]);
						}
						
						$el.table = {
							columns:columns,
							rows:rows,
						}
    					
						_processPdfPage($el,rowIndex);
					}else if (obj.type == 'i') {
						//Get Base64 IMG
						var url = $('#'+obj.id).prop('src');
						console.log(url);
						//Use this image for dev purposes.
						// var url = 'https://lh4.ggpht.com/wKrDLLmmxjfRG2-E-k5L5BUuHWpCOe4lWRF7oVs1Gzdn5e5yvr8fj-ORTlBF43U47yI=w300';
						convertFileToDataURLviaFileReader(url,rowIndex);
					}else if (obj.type == 'c') {
						// Convert to Chart to SVG then Base 64
						var dataURI = getImageData($('#'+obj.id).highcharts().getSVG());
						_processPdfPage(dataURI,rowIndex);
					}else if (obj.type == 't') {
						// Get Data
						// console.log(obj);
						var layer = obj.id.split('_')[1];
						// Table Data
						var rows = [];
						var dataTable = $scope.summaryData.layerSummary[layer].dataTableEntries;
						var dataLength = dataTable.length;
						var unitStr =  (typeof($scope.summaryData.layerSummary[layer].unit) != 'undefined') ?
		 				" (" + $scope.summaryData.layerSummary[layer].unit.trim() + ")" :
		 				"";
						var column1 = replaceAll($scope.summaryData.layerSummary[layer].layerName, "_", " ") +unitStr
					   	var columns = [column1, "Pixel Count (freq)" , "Acres"+$scope.aoiTag,"Percent"+$scope.aoiTag,];
						for (var i = 0; i < dataLength; i++) {
							// console.log(dataTable[i]);
							rows.push([
								dataTable[i].xLabel,
								dataTable[i].pixelCount,
								dataTable[i].acres,
								dataTable[i].percent,
							]);
						}
						// console.log(rows);
						_processPdfPage({
							columns:columns,
							rows:rows,
						},rowIndex);
					}
				};
			}
		});
	}

}
